...
#define MNH_ERR_BUFLEN   (PATH_MAX + 256)
...
static int mnh_file_is_zero(int fd, uint64_t size) {
    char buf[16384];
...
static int mnh_reserve(int fd, uint64_t size) {
    const char *sp = getenv("DATA_MINHASH_SHARED_SPARSE");
...
static MnhHandle *mnh_create(const char *path, uint64_t k, mode_t mode, char *errbuf) {
    ...
    if (anonymous) {
        ...
        if (is_new && mnh_reserve(fd, total) < 0) {
            MNH_ERR("%s: cannot reserve %llu bytes: %s", path, (unsigned long long)total, strerror(errno));
        ...
        if (!is_new) {
            if (!mnh_validate_header((MnhHeader *)base, (uint64_t)st.st_size)) {
                ...
                if (((MnhHeader *)base)->magic == 0 && (uint64_t)st.st_size == total
                    && st.st_uid == geteuid() && mnh_file_is_zero(fd, map_size)) {
                    if (fchmod(fd, mode) < 0) {
                        MNH_ERR("%s: fchmod: %s", path, strerror(errno));
                        munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                    }
                    if (mnh_reserve(fd, total) < 0) {
                        MNH_ERR("%s: cannot reserve %llu bytes: %s", path, (unsigned long long)total, strerror(errno));
                        munmap(base, map_size); flock(fd, LOCK_UN); close(fd); return NULL;
                    }
...
static MnhHandle *mnh_create_memfd(const char *name, uint64_t k, char *errbuf) {
    ...
    if (mnh_reserve(fd, total) < 0) {
        MNH_ERR("memfd: cannot reserve %llu bytes: %s", (unsigned long long)total, strerror(errno));
...
static MnhHandle *mnh_open_fd(int fd, char *errbuf) {
    ...
    uint32_t magic;
    if (pread(fd, &magic, sizeof magic, offsetof(MnhHeader, magic)) != (ssize_t)sizeof magic || magic != MNH_MAGIC) {
        MNH_ERR("invalid MinHash sketch table"); return NULL;
    }
...
static MnhHandle *mnh_open_readonly(const char *path, char *errbuf) {
    ...
    uint32_t magic;
    if (pread(fd, &magic, sizeof magic, offsetof(MnhHeader, magic)) != (ssize_t)sizeof magic || magic != MNH_MAGIC) {
        MNH_ERR("%s: invalid MinHash sketch file", path); close(fd); return NULL;
    }
    size_t ms = (size_t)st.st_size;
